...
namespace ofxWin32
{
    ...
    ofAppWin32Window::ofAppWin32Window(){
        timeNow				= 0;
        timeThen			= 0;
        ...
        nonFullScreenX		= -1;
        nonFullScreenY		= -1;
        nonFullScreenWidth  = -1;
        nonFullScreenHeight = -1;
        mouseX				= 0;
        mouseY				= 0;

    }

    void ofAppWin32Window::setupOpenGL(int w, int h, int screenMode){
        windowInitClass();
        windowInitDisplayMode(w, h, screenMode);
        windowCreateGLWindow(_T("OF"), w, h);
        windowInitDCRC();
        ...
        if (windowMode != OF_GAME_MODE){
            ...
            */
            ...
            requestedWidth  = nonFullScreenWidth = winmanGet(WINDOW_WIDTH);
            requestedHeight = nonFullScreenHeight = winmanGet(WINDOW_HEIGHT);
        } else {
            /*
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::initializeWindow(){


         //----------------------
         // setup the callbacks

         winmanMouseFunc(mouse_cb);
         winmanMotionFunc(motion_cb);
         winmanPassiveMotionFunc(passive_motion_cb);
         winmanIdleFunc(idle_cb);

         winmanDisplayFunc(display);
         winmanKeyboardFunc(keyboard_cb);
         winmanKeyboardUpFunc(keyboard_up_cb);
         winmanSpecialFunc(special_key_cb);
         winmanSpecialUpFunc(special_key_up_cb);

         winmanReshapeFunc(resize_cb);

    }

    //------------------------------------------------------------
    void ofAppWin32Window::runAppViaInfiniteLoop(ofBaseApp * appPtr){
    ...
    //	#endif
        ofLog(OF_LOG_VERBOSE,"Win32 OF app is being terminated!");

        OF_EXIT_APP(0);
    }

    //------------------------------------------------------------
    float ofAppWin32Window::getFrameRate(){
        return frameRate;
    }

    //------------------------------------------------------------
    int ofAppWin32Window::getFrameNum(){
        return nFrameCount;
    }

    void ofAppWin32Window::setWindowTitle(string title){
        TCHAR *tcTitle=new TCHAR[title.size()+1];
        tcTitle[title.size()]=0;
        ...
        copy(title.begin(), title.end(), tcTitle);
        winmanSetWindowTitle(tcTitle);
    }

    //------------------------------------------------------------
    ofPoint ofAppWin32Window::getWindowSize(){
        int width = winmanGet(WINDOW_WIDTH);
        int height = winmanGet(WINDOW_HEIGHT);
        return ofPoint(width, height,0);
    }

    //------------------------------------------------------------
    ofPoint ofAppWin32Window::getWindowPosition(){
        int x = winmanGet(WINDOW_X);
        int y = winmanGet(WINDOW_Y);
        return ofPoint(x,y,0);
    }

    //------------------------------------------------------------
    ofPoint ofAppWin32Window::getScreenSize(){
        int width = winmanGet(SCREEN_WIDTH);
        int height = winmanGet(SCREEN_HEIGHT);
        return ofPoint(width, height,0);
    }

    //------------------------------------------------------------
    void ofAppWin32Window::setWindowPosition(int x, int y){
        winmanPositionWindow(x,y);
    }

    //------------------------------------------------------------
    void ofAppWin32Window::setWindowShape(int w, int h){
        winmanReshapeWindow(w, h);
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::hideCursor(){
        ShowCursor(FALSE);
    }

    //------------------------------------------------------------
    void ofAppWin32Window::showCursor(){
        ShowCursor(TRUE);
    }

    //------------------------------------------------------------
    void ofAppWin32Window::setFrameRate(float targetRate){
    ...
    //------------------------------------------------------------
    int ofAppWin32Window::getWindowMode(){
        return windowMode;
    }

    //------------------------------------------------------------
    void ofAppWin32Window::toggleFullscreen(){
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::setFullscreen(bool fullscreen){
        if( windowMode == OF_GAME_MODE)return;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::enableSetupScreen(){
        bEnableSetupScreen = true;
    }

    //------------------------------------------------------------
    void ofAppWin32Window::disableSetupScreen(){
        bEnableSetupScreen = false;
    }

    //------------------------------------------------------------
    void ofAppWin32Window::display(void){
        static ofEventArgs voidEventArgs;
        ...
        if (windowMode != OF_GAME_MODE){
            if ( bNewScreenMode ){
                if( windowMode == OF_FULLSCREEN){
                    ...
                    nonFullScreenX = winmanGet(WINDOW_X);
                    nonFullScreenY = winmanGet(WINDOW_Y);
                    nonFullScreenWidth = winmanGet(WINDOW_WIDTH);
                    nonFullScreenHeight = winmanGet(WINDOW_HEIGHT);
                    //----------------------------------------------------
                    ...
                    winmanReshapeWindow(nonFullScreenWidth, nonFullScreenHeight);

                    //----------------------------------------------------
                    // if we have recorded the screen posion, put it there
                    // if not, better to let the system do it (and put it where it wants)
                    if (nFrameCount > 0){
                        winmanPositionWindow(nonFullScreenX,nonFullScreenY);
                    }
                    //----------------------------------------------------
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::mouse_cb(int button, int state, int x, int y) {
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::motion_cb(int x, int y) {
        static ofMouseEventArgs mouseEventArgs;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::passive_motion_cb(int x, int y) {
        static ofMouseEventArgs mouseEventArgs;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::idle_cb(void) {
        static ofEventArgs voidEventArgs;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::keyboard_cb(unsigned char key, int x, int y) {
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::keyboard_up_cb(unsigned char key, int x, int y) {
        static ofKeyEventArgs keyEventArgs;
    ...
    //------------------------------------------------------
    void ofAppWin32Window::special_key_cb(int key, int x, int y) {
        static ofKeyEventArgs keyEventArgs;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::special_key_up_cb(int key, int x, int y) {
        static ofKeyEventArgs keyEventArgs;
    ...
    //------------------------------------------------------------
    void ofAppWin32Window::resize_cb(int w, int h) {
