...
/**
 * Update the locations of the glints. Sets glintID to -1 if glint not found,
 * otherwise glintID is set to the index of glint in the blobs of the contour finder
 */
void glintLineChecker::update(ofxCvGrayscaleImage & eyeImg, int nGlints, ofxCvContourFinder & contourFinder, bool bUseGlintInBrightEye, ofxCvContourFinder & contourFinderBright){
	...
	if (nGlints == 2) {
        ...
        /*
         * Create a vector of lines containing the black rows of the
         * binary image that lie in between the glints.
         */
        ...
        /*
         * Remove lines that are not in range. Lines
         * not in range are lines in which the distance
         * (i.e. the length of the line) exceeds the max
         * distance or the distance is smaller than the min
         * distance.
         */
		if (bDeleteLine) {
