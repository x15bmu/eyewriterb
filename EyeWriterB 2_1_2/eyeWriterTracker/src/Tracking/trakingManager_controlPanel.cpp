...
void trackingManager::setupGui(){
    ...
    bool videoSettingsPanel = false;
	if(IM.mode == INPUT_LIVE_VIDEO && (IM.grabberType == INPUT_OFXLIBDC || IM.grabberType == INPUT_OFXCLEYE))
	{
        panel.addPanel("video settings", 1, false); //image adjustment panel gets way too long otherwise
        videoSettingsPanel = true;
	}
	...
	panel.addSlider("Overlay Opacity", "OVERLAY_OPACITY", 120, 0, 255, true);
	panel.addSlider("Key Press Time (Seconds)", "KEY_PRESS_TIME", 1.5, 0.5, 3, false);
	panel.addSlider("Click Time (Seconds)", "CLICK_TIME", 1.5, 0.5, 3, false);

    if(videoSettingsPanel)
    {
        panel.setWhichPanel("video settings");
#ifdef TARGET_OSX
...
#ifdef TARGET_WIN32
        if(IM.mode == INPUT_LIVE_VIDEO && IM.grabberType == INPUT_OFXCLEYE) {
            clVideoGrabber& cam = *((clVideoGrabber*) IM.vidGrabber);
            panel.addToggle("auto gain", "INPUT_CL_AUTO_GAIN", cam.getAutoGain());
            panel.addSlider("manual gain", "INPUT_CL_GAIN", cam.getGain(), 0, 79, false);
            panel.addToggle("auto exposure", "INPUT_CL_AUTO_EXPOSURE", cam.getAutoExposure());
            panel.addSlider("manual exposure", "INPUT_CL_EXPOSURE", cam.getExposure(), 0, 511, false);
        }
#endif
    }
...
void trackingManager::updateGui(){
    ...
    if(IM.mode == INPUT_LIVE_VIDEO && IM.grabberType == INPUT_OFXCLEYE)
    {
        bool autoGain = panel.getValueB("INPUT_CL_AUTO_GAIN");
        int gain = panel.getValueI("INPUT_CL_GAIN");
        bool autoExposure = panel.getValueB("INPUT_CL_AUTO_EXPOSURE");
        int exposure = panel.getValueI("INPUT_CL_EXPOSURE");

        clVideoGrabber& cam = *((clVideoGrabber*)IM.vidGrabber);
        if(autoGain != clLastAutoGain)
        {
            cam.setAutoGain(autoGain);
            clLastAutoGain = autoGain;
            if(!autoGain)
                clLastGain = gain + 1; //force update gain if autogain just got turned off
        }
        if(!autoGain && gain != clLastGain)
        {
            cam.setGain(gain);
            clLastGain = gain;
        }
        if(autoExposure != clLastAutoExposure)
        {
            cam.setAutoExposure(autoExposure);
            clLastAutoExposure = autoExposure;
            if(!autoExposure)
                clLastExposure = exposure + 1; //force update exposure if autoexposure just got turned off
        }
        if(!autoExposure && exposure != clLastExposure)
        {
            cam.setExposure(exposure);
            clLastExposure = exposure;
        }
